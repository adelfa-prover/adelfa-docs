...
You can follow along using the specification file [`stlc.lf`](/stlc.lf) and theorem
development [`stlc.ath`](/stlc.ath).

## Canonical LF Specification

The following Canonical LF signature will encode typing and evaluation in the
simply typed $\lambda$-calculus and form the basis for our reasoning.
...
will split the proof into two branches based on the two possible ways in which `E`
can evaluate to `V`. Each of the different proof branches is called a subgoal. In
general, only the first subgoal will be displayed in full; the other subgoals
will be shown without their hypotheses. When the first subgoal is completed, the
prover will move to the next subgoal.
