---
title: Transporting Theorems Across Context Schemas
...
understanding of the syntax and tactics of Adelfa. We recommend the
reader explore the provided [walkthrough](/walkthrough) or have the [reference
guide](/reference-guide/syntax) at hand. You may also search using the search bar in
the top left to look up terms.

If you'd like to install Adelfa and try things out for yourself, you must
install the latest version from
[GitHub](https://github.com/adelfa-prover/adelfa) which implements the transport
rule.
...
We discuss a series of examples which utilize _schema subsumption_, a
...
Theorems about Typeability in LF Across Schematically Defined
Contexts"[@johnson2025ppdp] but aim to be as self-contained as possible. We
suggest reading the following examples in the order presented below as they are
intended to build on each other.
...
false.` in sections discussing how the proof would proceed without using the
transport rule.

1. [Every lambda calculus term has a size](/theory/schema-subsumption/size) displays a
...
2. [Size of lambda calculus terms does not decrease after
   substitution](/theory/schema-subsumption/subst-size) shows the use of transport in
   a larger development order to bring out how avoiding transport causes more
   work as one's development grows.
3. [ORBI](/theory/schema-subsumption/orbi) completes a standardized benchmark for
   what is referred to as "Non-Linear Context Extensions," which schema
   subsumption permits.
4. [POPLmark](/theory/schema-subsumption/poplmark) utilizes schema subsumption across
   schemas with multiple blocks.

[A note on Beluga's schema subsumption](/theory/schema-subsumption/beluga)
