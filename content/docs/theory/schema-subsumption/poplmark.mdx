...
- Signature: [`1a.lf`](/poplmark/1a.lf)
- Proof development: [`1a.ath`](/poplmark/1a.ath)
- CMU Twelf solution: [`cmu-twelf.tar.gz`](/poplmark/cmu-twelf.tar.gz)
...
<details>
  <summary>
    ...
    Since the cases for `bound_var` depend on assumptions that are on only
    ...
    will also add `bound_var` assumptions.  Hence, we have the following
    ...
    assumptions.  However, the `bound_var` assumption applies, and states
...
## Simplification via Schema Subsumption
...
every case in which $\mathcal{C}_{\mathrm{all}}$ was a series of repetitions of
