...
Our usage of the transport rule in [term has size](/theory/schema-subsumption/size)
...
size](/theory/schema-subsumption/size) example, reiterated here:
...
- Proof development _with_ transport:
  [`size-transport.ath`](/subst-size/subst-size-transport.ath)
...
>     lemma, the size of a term must be at least one.
...
>   not occur freely within $e$, then this follows from the inductive hypothesis on
>   $e$.
...
To specify this statement in LF, we extend the specification from
[term has size](/theory/schema-subsumption/size#specification) to include rules for less
...
proof in its entirety here, but omit state to reduce the page's size.
