...
- $\mathtt{eval\textrm{-}app}$ evaluates the first term of the application,
  which results in an abstraction to permit the application. Then the second term
...
- $\mathrm{of\textrm{-}var}$ ensures that when we encounter a variable, it
...
For example, the evaluation rules relate two terms. Therefore, we introduce a
...
U}`. This judgement analyzes the type of `R` when `x` is of type `T`. The
...
We have finished our encoding of the object system, and can now begin to reason
...
deterministic. In other words, multiple evaluations of the same term always result in
equivalent results.
