...
description: Syntax, commands, and tactics of the adelfa proof assistant
---
...
The syntax for Canonical LF terms and types follows a similar style to
...
- `search_depth` The maximum number of times to
  ...
  [`search`](#search) and
  [`assert`](#assert) tactics.
  Accepts any number as a `<VALUE>`. By default, this is set to 5.
- `schema_sub` The option to enable or disable [schema
  subsumption](/theory/schema-subsumption/introduction). Accepts `true` or `false`. By default, this is set to `true`.
...
Restrictions](#inductive-restrictions).

Adelfa supports nested induction through repeated calls to the induction tactic.
See the [Inductive Restrictions](#inductive-restrictions)
