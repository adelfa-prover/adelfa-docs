...
then
    ...
    rsync -arzv -p --chmod=ug+rwX,o+rX --exclude 'Makefile' --exclude 'examples/Makefile' --exclude 'examples/*/Makefile' --include='*.html' --include='*.ath' --include='*.lf' --include='*.css' --include='*.tar' --include='*/' --exclude='*' --prune-empty-dirs . $WEBSITE_URL:$WEBSITE_REL_PATH
fi
