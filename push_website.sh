...
then
    ...
    rsync -arzv -p --chmod=ug+rwX,o+rX --exclude 'code/Makefile' --include='*.html' --include='*.thm' --include='*.lf' --include='*.css' --include='*.tar' --include='*/' --exclude='*' --prune-empty-dirs . $WEBSITE_URL:$WEBSITE_REL_PATH
fi
