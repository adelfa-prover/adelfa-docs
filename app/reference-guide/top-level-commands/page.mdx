...
- `search_depth` <a id="search_depth"></a> The maximum number of times to
  perform type decomposition on assumption judgements during the
  [`search{:adelfa}`](/reference-guide/tactics#search-num) and
  [`assert{:adelfa}`](/reference-guide/tactics#assert-formula-num) tactics.
  Accepts any number as a `<VALUE>`. By default, this is set to 5.
- `schema_sub` <a id="schema_sub"></a> The option to enable or disable [schema
