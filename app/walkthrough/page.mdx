...
In this example, we will reason about the $\lambda$-calculus with big step (sometimes
referred to as "call-by name") evaluation semantics. We denote this
relation as a relation between two terms $e_1 \Downarrow e_2$. The "input" term
is on the left of the $\Downarrow$ and the "output" term is on the right.
